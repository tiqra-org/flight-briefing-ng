...
export const regexPatternValidator = (pattern: RegExp, errors: ValidationErrors): ValidatorFn => {
...
export function atLeastOneCheckboxRequired(): ValidatorFn {
	return (control: AbstractControl): ValidationErrors | null => {
		if (control instanceof FormArray) {
			const checkedKeys = Object.keys(control.controls).filter((key, index) => control.controls[index].value);
...
export function atLeastOneFieldRequired(controlNames: string[]) {
	return (group: AbstractControl) => {
		if (controlNames.map(name => group.get(name)).filter(c => c?.value).length === 0) {
